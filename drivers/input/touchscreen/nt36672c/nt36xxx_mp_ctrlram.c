				     ...
				     int32_t *item_array)
{
	...
	*ptr = tmp;
	...
	while ((token = strsep(&tok_ptr, ", \t\r\0"))) {
		if (strlen(token) == 0)
			continue;
		item_array[Y_Channel * X_Channel + i] =
			(int32_t)simple_strtol(token, NULL, 10);
...
static int32_t nvt_load_mp_setting_criteria_from_csv(const char *filename)
{
	...
	*/
	...
	ptr = fbufp;
	if (parse_mp_setting_criteria_item(
		    &ptr, "IC_KEY_CFG_SIZE:", &IC_KEY_CFG_SIZE) < 0) {
		retval = -1;
		goto exit_free;
	}
#endif /* #if TOUCH_KEY_NUM > 0 */
...
#if TOUCH_KEY_NUM > 0
	ptr = fbufp;
	if (parse_mp_setting_ain_array(&ptr, "AIN_KEY:", AIN_KEY,
				       IC_KEY_CFG_SIZE) < 0) {
		retval = -1;
		goto exit_free;
	}
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	snprintf(mp_setting_criteria_csv_filename,
		 sizeof(mp_setting_criteria_csv_filename),
...
const struct seq_operations t_fops = { .start = t_start,
				       .next = t_next,
				       .stop = t_stop,
				       .show = t_show };
